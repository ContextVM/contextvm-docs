...
`OpenStreamConfig` lives in `contextvm_sdk::transport::open_stream`, not at the crate root. `OpenStreamConfig::enabled()` is the same as `OpenStreamConfig::default().with_enabled(true)`: enabled with every other knob at its default. Once enabled the capability is safe for non-CEP-41 peers, because the server activates a stream only for clients that advertised support, and injects a writer only when a request carries a `progressToken`.

## Server side: emitting a stream from a tool

When open-stream is enabled and an incoming `tools/call` carries a `progressToken`, the transport constructs an `OpenStreamWriter` for that request and inserts it into the rmcp request extensions before the handler runs. A tool handler that wants to stream retrieves the writer from `ctx.extensions`:
...
`call_tool_stream` returns a `ToolStreamCall` with four parts:

- `progress_token`: the stringified token that correlates the call and its stream
- `stream`: an async `Stream` of `Result<String, OpenStreamError>` chunks
- `result`: a future resolving to the final `CallToolResult` after the stream closes
- `abort()`: a method that cancels the call

`call_tool_stream` and `ClientOpenStreamHandle` are re-exported at the crate root; the chunk error type `OpenStreamError` lives in `contextvm_sdk::transport::open_stream`.

To cancel from the consumer side, call `call.abort(Some("reason".to_string())).await`. That publishes an `abort` frame to the server so its writer stops, finalizes the local stream, and frees the reader slot.

## The timeout model

A reader protects itself against a stream that goes silent with three timers, all configurable on `OpenStreamConfig`:

- **idle timeout** (`idle_timeout_ms`, default `30000`): how long the reader waits without any frame before it probes the peer with a `ping`. Every inbound frame resets it, so a live stream never trips it.
- **probe timeout** (`probe_timeout_ms`, default `20000`): how long the reader then waits for a `pong` before it gives up and aborts the stream.
- **close grace period** (`close_grace_period_ms`, default `5000`): applies after a `close` arrives with buffered gaps still unresolved — the reader waits this long for the missing chunks before aborting.

There is no hard lifetime cap by default; an open stream may legitimately run for a long time. Set `max_total_timeout_ms` to `Some(ms)` if you want one. `call_tool_stream` also derives the rmcp request timeout from these values, summing idle, probe, and close-grace so the rmcp request is never failed before the keepalive logic would have aborted a genuinely dead stream, and it re-arms that timeout on every forwarded frame.

## Known limitation: keep the final response small

The final `CallToolResult` of a streamed call is delivered on a deferred path that publishes it as a single relay event. That path does not apply CEP-22 fragmentation. A normal, non-streamed response at or above the oversized threshold (48000 bytes by default) is split into frames and reassembled, but the deferred final response of a streamed call is not, so it must fit within a single relay event (the same single-event ceiling described in the [oversized transfer guide](/reference/rs-sdk/oversized-transfer), roughly 64 KiB on the wire).

Keep the final result small and let the bulk of the payload ride the stream as chunks.

## Configuration reference

`OpenStreamConfig` is attached to both transport configs via `with_open_stream(..)`. All fields:
...
Each field has a `with_*` builder (`with_enabled`, `with_max_concurrent_streams`, `with_max_buffered_chunks_per_stream`, `with_max_buffered_bytes_per_stream`, `with_idle_timeout_ms`, `with_probe_timeout_ms`, `with_close_grace_period_ms`, `with_max_total_timeout_ms`), so you can start from `OpenStreamConfig::enabled()` and override individual knobs.

## Related documentation

- [CEP-41: Open-Ended Streams](/reference/ceps/cep-41)
- [Oversized Transfer](/reference/rs-sdk/oversized-transfer)
- [Native Client Guide](/reference/rs-sdk/client-transport)
- [Native Server Guide](/reference/rs-sdk/server-transport)

---

_This page was ported from the [ContextVM Rust SDK repository](https://github.com/ContextVM/rs-sdk/tree/main/docs/open-stream.md)._
