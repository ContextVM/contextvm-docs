...
description: Diagnose and resolve common ContextVM connection issues, relay problems, encryption failures, and authentication errors.
---
...
|-------|-------|---------- |
| "Event not found" or timeout | Request event ID doesn't exist; subscription filter too restrictive; relay dropped connection | Verify event ID; check filters; reconnect to relay |
| "Invalid signature" | Private key doesn't match claimed public key; event was modified after signing; signing implementation error | Verify key pair; check signing code |
| "Method not found" | Request method name is wrong; server doesn't support that capability; server hasn't announced that tool/resource/prompt | Check method name; verify server capabilities |
| "Connection refused" | Relay is down or URL is wrong; firewall blocking connection; incorrect WebSocket URL format | Check relay URL; test connectivity; verify URL format |
| "Payment required" | Server requires payment for this capability; client hasn't paid | Check [Payments](/cvmi/skills/payments) documentation |
...
```bash
LOG_LEVEL=debug cvmi serve ...
LOG_LEVEL=debug cvmi use ...
```
...
1. Check the [ContextVM documentation](https://docs.contextvm.org)
2. Review relevant [CEPs](/spec/ceps) for protocol details
3. Search existing issues on [GitHub](https://github.com/contextvm)
4. Join the community for support

## Related Resources

- [CEP-4](/spec/ceps/cep-4) — Encryption specification
- [Deployment](/cvmi/skills/deployment) — Production deployment
- [Server Development](/cvmi/skills/server-dev) — Server development
- [Client Development](/cvmi/skills/client-dev) — Client development
